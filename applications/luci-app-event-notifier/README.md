...
- `logread -f` syslog parsing for login, LuCI login, firewall/security, service health, DHCP lease, VPN, SIM service and DNS/network events.
- `/etc/hotplug.d/` hooks or ubus events for hotplug event capture.
- netifd/ubus events for interface state capture.
- netlink carrier state or netifd events for Ethernet plug/unplug capture.
